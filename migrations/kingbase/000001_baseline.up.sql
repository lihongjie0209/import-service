CREATE TABLE import_jobs (
    ...
    invalid_rows bigint NOT NULL DEFAULT 0, applied_rows bigint NOT NULL DEFAULT 0, progress_percent integer NOT NULL DEFAULT 0,
    error_code text NOT NULL DEFAULT '', error_message text NOT NULL DEFAULT '', idempotency_key text NOT NULL,
    confirm_key text NOT NULL DEFAULT '', upload_expires_at timestamptz, started_at timestamptz,
    completed_at timestamptz, result_expires_at timestamptz, version bigint NOT NULL DEFAULT 1,
...
CREATE INDEX import_jobs_queued_idx ON import_jobs (status, created_at, id);
CREATE INDEX import_jobs_expiry_idx ON import_jobs (status, result_expires_at);
...
CREATE INDEX import_outbox_pending_idx ON import_outbox_events (published_at, available_at, created_at);
