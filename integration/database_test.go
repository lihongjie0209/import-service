...
func TestRepositoryAndMigrations(t *testing.T) {
	for _, databaseType := range []string{"postgres", "mysql"} {
		t.Run(databaseType, func(t *testing.T) {
			...
			var importTables int
			if databaseType == "postgres" {
				if err := db.GetContext(ctx, &importTables, `SELECT count(*) FROM pg_tables WHERE schemaname = current_schema() AND tablename = 'import_jobs'`); err != nil {
					t.Fatal(err)
				}
				var timezone string
			...
			} else if err := db.GetContext(ctx, &importTables, `SELECT count(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'import_jobs'`); err != nil {
				t.Fatal(err)
			}
			if importTables != 1 {
