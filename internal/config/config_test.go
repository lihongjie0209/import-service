...
func TestLoad_IdempotencyRouteEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("APP_IDEMPOTENCY_HTTP_PATHS", "[/api/v1/imports/complete-upload, /api/v1/imports/confirm]")
	t.Setenv("APP_IDEMPOTENCY_GRPC_METHODS", "[/platform.import.v1.ImportService/ConfirmImportJob]")
	cfg, err := Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if got := strings.Join(cfg.Idempotency.HTTPPaths, ","); got != "/api/v1/imports/complete-upload,/api/v1/imports/confirm" {
		t.Fatalf("HTTPPaths=%q", got)
	}
	if got := strings.Join(cfg.Idempotency.GRPCMethods, ","); got != "/platform.import.v1.ImportService/ConfirmImportJob" {
