...
func TestCatalogListsNormalizedPageWithinTenant(t *testing.T) {
	...
	values, total, page, size, err := NewCatalog(provider).List(userContext("tenant-1"), "tenant-1", "app-1", " users ", 0, 0)
	if err != nil || len(values) != 1 || total != 1 || page != 1 || size != 20 {
		t.Fatalf("values=%+v total=%d page=%d size=%d err=%v", values, total, page, size, err)
	}
	if _, _, _, _, err := NewCatalog(provider).List(userContext("tenant-2"), "tenant-1", "app-1", "", 1, 20); err == nil {
...
func TestCatalogDescribesProviderDataset(t *testing.T) {
	t.Parallel()
	provider := providerStub{describe: func(tenant, application, service, dataset string) (DatasetDescriptor, error) {
		if tenant != "tenant-1" || application != "app-1" || service != "identity-service" || dataset != "identity.users" {
			t.Fatalf("tenant=%q application=%q service=%q dataset=%q", tenant, application, service, dataset)
		}
		return DatasetDescriptor{Code: dataset, Title: "Users"}, nil
	}}
	value, err := NewCatalog(provider).Describe(userContext("tenant-1"), "tenant-1", "app-1", "identity-service", "identity.users")
