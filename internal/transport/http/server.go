...
import (
	...
	docs "github.com/lihongjie0209/import-service/docs"
	...
	"github.com/lihongjie0209/import-service/internal/ratelimit"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, limiter *ratelimit.Limiter, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	imports.POST("/list", handler.ListImports)
	imports.POST("/cancel", handler.CancelImport)
	imports.POST("/retry", handler.RetryImport)
	imports.POST("/confirm", handler.ConfirmImport)
	imports.POST("/error-report", handler.DownloadErrorReport)
