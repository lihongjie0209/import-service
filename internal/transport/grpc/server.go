...
import (
	...
	"github.com/lihongjie0209/import-service/internal/environment"
	apphealth "github.com/lihongjie0209/import-service/internal/health"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, authService *auth.Service, authorizer platformauthz.Authorizer, healthService *apphealth.Service, importService *importjob.Service, catalog *importjob.Catalog, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, logger *slog.Logger) (*Server, error) {
	...
	importv1.RegisterImportServiceServer(grpcServer, &importServer{service: importService, catalog: catalog})
...
func authenticateGRPC(ctx context.Context, method string, service *auth.Service, cfg config.Auth) (context.Context, error) {
	...
	if cfg.PSK.Enabled && auth.MatchesAny(method, cfg.PSK.GRPCMethods) {
		...
		return platformprincipal.WithContext(ctx, platformprincipal.Principal{ID: "import-service:psk", Type: platformprincipal.TypeServiceAccount}), nil
	}
